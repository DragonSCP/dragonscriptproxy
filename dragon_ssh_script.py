...
def load_json_from_url(url):
    try:
        ...
        print("Erro: Falha ao decodificar o JSON do conteúdo do URL.")
        return {}

def generate_and_test_proxies_from_base_ip():
    base_ip = input("Insira o IP base (ex: 192.168.1.): ")
    start_range = int(input("Comece do número: "))
    end_range = int(input("Até o número: "))
    for i in range(start_range, end_range + 1):
        ip = f"{base_ip}{i}"
        result, external_ip = test_proxy(ip, 80)
        if result:
            print(f"Proxy {ip}:80 funcionou! IP Externo: {external_ip}")
        else:
            print(f"Proxy {ip}:80 não funcionou.")

def fetch_payload_data():
    url = "https://raw.githubusercontent.com/DragonSCP/dragonscriptproxy/main/payload_config.json"
    global payload_data
    payload_data = load_json_from_url(url)
    if payload_data:
...
def cleanup_and_exit():
    files_to_remove = ['dragon_ssh_script.py']  # Adicione aqui outros arquivos se necessário
    ...
    # Você pode adicionar aqui mais comandos para limpar diretórios ou outros arquivos específicos
...
def main():
    ...
    while True:
        ...
        if choice == '':
            ...
            # Aqui você pode inserir a lógica para testar proxies por operadora e IP
            pass
        elif choice == '5':
            update_script()
        else:
